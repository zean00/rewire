...
### 2026-09-25 — ENTRY L LAUNCHED (authorized): seeds, runner, and the completion-claim rule frozen before session 1; infra up per the pinned recipe; sanity first, then the 168-session confirmation sweep.

**Seeds (drawn at launch, before any fresh session exists).** SEED_RNG **9537397989**, sanity seed **7523903923**, both from the OS CSPRNG immediately before this entry. Runner `run_v5.py` = `run_v4.py` verbatim except the four frozen deltas: (1) seed RNG 9537397989; (2) seed exclusion now covers every v1/v2/v3/v4 seed; (3) v5 output paths (`main_v5.jsonl`, `main_runs_v5/`, CDP journal, `sanity_runs_v5/`); (4) sanity seed. Task list (24), runner prompt, arm (`chain`), model string (`poc-proxy-12b/gemma-12b-decide`), caps (480 s omp / 120 s page), CDP collection, and the frozen label rule (raw > 0 passes; None/no page state = failure) unchanged. Seed plan printed to `eval/reports/entry_l/seed_plan_v5.txt` before any session runs.

**Completion-claim rule (frozen here; mechanically validated on old data only — counts, no scores).** The confirmation pool is the GATE-firing population. After the sweep: probes extracted by the frozen ckpt_review machinery verbatim; per session the pool takes **the LAST review probe by (turn, evidence_turn)** — the completion-claim moment — and its state block is built by the frozen build_evidence semantics; label = the session's page reward. **No polarity filter**: the dial reads evidence, not claim vocabulary (that substitution is the integration). Validation of the rule on the v4 extraction (old data): 897 raw probes, **140 of 168 sessions fire the GATE at least once** — the 28 gate-less sessions are outside the dial's jurisdiction and are disclosed by count at adjudication; pool = sessions with ≥ 1 review. v4 pool shape under this rule: 140 blocks, 20 positive (base rate 0.1429, constant Brier 0.1224) — recorded for expectation only; **the fresh pool's own constant is recomputed at adjudication, never assumed**.

**Gate (verbatim from entry L).** AUROC ≥ 0.75 AND Brier < the fresh pool's own constant predictor, one adjudication pass, scored with `entry_k_readout.json` (sha c840ba54…323e8) verbatim; the operational confusion at tau* = 0.326134 (the accept/reject point the GATE routes on) is recorded alongside. No tuning of anything after any fresh metric is seen.

**Infra (relaunched this session per the pinned entry-J launch recipe, before the sanity session).** Generation llama-server on `127.0.0.1:8998` (`gemma-4-12b-it-nvfp4.gguf`, `--jinja -c 65536 -ctk q8_0 -ctv q8_0 -ngl 99 --flash-attn on -b 1024 -ub 512`, loopback only — the film-worker keeps the Tailscale listener) and the chain proxy on `:8990` (`eval/omp_proxy.py --port 8990 --backend-config eval/proxy_chain_remote_review.json --no-local-model`, frozen gates `configs/webgate_remote.json`). Standing 4B proxy (8999) and film-worker untouched. One sanity session (throwaway seed 7523903923) must land a page reward with a clean transcript before the sweep starts. Expected sweep wall from the v4 record: **~10.5 h** (mean 226.3 s/session × 168), serial and resumable; the sweep runs unattended.

**Teardown commitment (after adjudication).** Both sweep servers stopped by ps-verified PID, each in its own ssh call; standing 4B proxy and film-worker untouched; VRAM verified back to baseline. Artifacts: seed plan, sanity record, sweep label file, probe/state/embedding files, predictions, and results — all sha-pinned and committed.
### 2026-09-25 — ENTRY M PRE-REGISTERED (before any Qwen-quant embedding exists): the precision ladder's first rung — Qwen3-8B Q4_K_M through the same llama-server embeddings recipe, refit with the same dial, scored on the same pools. A diagnostic that ATTRIBUTES the J-vs-K gap; it earns nothing.

**Question.** The J readout (Qwen3-8B bf16 via HF, fresh AUROC 0.9889) and the K readout (Gemma-12B nvfp4 via llama-server, fresh AUROC 0.8867) differ in three confounds at once: backbone, weight precision, and numerics stack. This entry removes two of the three: the SAME Qwen3-8B backbone, quantized to a 4-bit-class GGUF, served by the SAME llama-server embeddings recipe that produced the K vectors, with the SAME dial recipe refit on the SAME 316 and applied cold to the SAME 290. Outcomes: ≈ 0.99 → precision/stack exonerated, the gap is Gemma's own representation; ≈ 0.89 → 4-bit-class quantization accounts for the gap, and the fix candidate becomes serving less-quantized weights (a chain-cost trade, separately registered).

**Why Q4_K_M and not NVFP4-exact.** No Qwen3-8B NVFP4 GGUF exists (searched 2026-09-25; public NVFP4 GGUFs are 27B-class). Q4_K_M is the same bit-width class (~4.8 bpw effective); the format mismatch (block-scaled k-quants vs nvfp4's E2M1+E4M3 two-level scales) is recorded, not hidden. The int8 rung (Q8_0, ~8.5 GB) is registered as the follow-up rung and runs on the GPU host AFTER entry L adjudicates — it fits neither beside the sweep's 12B server (8.5+8.5 > 15.9 GB) nor whole on this laptop's 8 GB card.

**Where it runs (disclosed).** This laptop's RTX 5060 Laptop GPU (8 GB) — NOT the GPU host, whose card is running the entry-L confirmation sweep until it adjudicates. Disclosed differences from the K recipe's serving context: different card (same Blackwell sm_120 family) and context 8192 × 1 slot instead of 32768 ÷ 4 (state texts are ≤ ~2000 tokens by the extraction cap; KV sized for the 8 GB card). The dial is refit on whatever this server produces, so the card is part of the instrument definition; the determinism recheck runs on this same card.

**Extraction/embedding recipe (frozen).** State texts byte-identical to entries H/J/K (`entry_h_states.jsonl` 8a684a05…48dd01, `entry_j2_states.jsonl` 4aa45387…d233); no re-extraction. llama-server (stock, local CUDA build of llama.cpp; commit hash recorded in the manifest) launched as: `llama-server -m models/Qwen_Qwen3-8B-Q4_K_M.gguf --alias qwen3-8b-q4 --host 127.0.0.1 --port 8998 --embedding --pooling last -c 8192 -np 1 -ngl 99 --flash-attn on -b 1024 -ub 512`. Vectors via POST /v1/embeddings, text verbatim, **one text per POST, strictly serial** (the entry-K standing instrument rule), ONE pass over all 606 texts; determinism recheck = 8 sampled rows re-embedded singly, bit-exact required; manifest records the server line, build commit, dim, counts, wall, recheck. Before any state text is embedded, a throwaway-string smoke verifies the build (dimension 4096, unit norm) and re-tests the entry-K batch-composition defect on this build (same text alone vs inside a two-text batch).

**Arms (verbatim transplants).** K-a and K-b exactly as entry K (same frozen folds `entry_j_folds.json` fe061388…818a, same seeds), fitted on the new Qwen-Q4 vectors, applied cold to the 290. Numbers are reported against the same bar (AUROC ≥ 0.75, Brier < 0.0982) **for comparability only — this is a diagnostic on a pool already adjudicated twice (entries J and K); it earns no deployment right and ships nothing.** The fit runs once on the GPU host's python env (this laptop lacks sklearn), on sha-pinned vectors scp'd over.

**Pre-committed violations.** Any recipe change after any metric is seen; a second embedding pass per rung (standing crash-fix exception: disclosed fix before any metric); re-extracting or editing state texts; tuning anything on v4; presenting these numbers as deployment evidence.

**Cost.** ~5 GB download, one local CUDA build, one 606-text embeddings pass (minutes), one fit on the host. Zero interference with the entry-L sweep: different machine, build niced, download throttled.
### 2026-09-25 — ENTRY M PASS-1 DETERMINISM RECHECK FAILED → DISCLOSED INSTRUMENT FIX (standing crash-fix exception, exercised BEFORE any metric exists; exactly one re-run follows).

Pass 1 (client already singles/serial per the standing rule; server = stock build of llama.cpp commit 4de09265, RTX 5060 Laptop): extraction of all 606 vectors completed in 55.1 s, but the 8-row recheck FAILED. Diagnosis (entry_m_diag.py): the failure is NOT the entry-K batch defect — smoke showed alone-vs-batched cos 0.99997 on this build — it is REPEAT-REQUEST nondeterminism: the same text re-embedded back-to-back returned vectors at cos 0.9996–0.9999 (max|d| up to 7.6e-3 on a unit vector). Cause: the server's cross-request prompt/KV cache (default `--cache-ram 8192`) serves some requests from cached KV and others from fresh compute — different floating-point reduction orders. Additionally, the very first request after server start runs off the CUDA-graph-capture path (cold) and differs from steady-state graph replay.

Fix (disclosed before any fit, metric, or threshold exists; server line changes only here): relaunch with `--cache-ram 0` (cross-request cache disabled; `--cache-reuse` already 0) and issue one throwaway warmup request so the pass runs entirely in steady-state graph replay. Steady-state spot check passed: A/B/A pattern bit-exact. Pass-1 artifacts preserved as evidence (`entry_m_emb_pass1.npz`, `entry_m_embed_manifest_pass1.json`, `determinism_recheck_bitexact: false`); the fit script's `assert recheck is True` guarantees pass-1 data can never enter adjudication. Exactly one re-run of the full 606-text pass; its result stands. Model artifact pinned in the process: Qwen_Qwen3-8B-Q4_K_M.gguf sha256 54fffa05…cccd (matches upstream-published hash).
### 2026-09-25 — ENTRY M INSTRUMENT FIX, PART 2 (complete account; standing crash-fix exception, still BEFORE any metric exists): the pass-1 fix was incomplete; three stacked history-dependence sources were found, fixed, and validated; one further full pass follows.

Pass-2 (server `--cache-ram 0` only) also FAILED its recheck. Source-level diagnosis on the local build (llama.cpp commit 4de09265, CUDA 13.3, sm_120) found three independent mechanisms, each of which makes an embeddings result depend on request history:

1. **Slot LCP KV keep.** Embedding tasks never inherit the `--no-cache-prompt` flag: `handle_embeddings_impl` builds tasks from struct defaults (`server_task.h`: `cache_prompt = true`), so the slot kept up to ~569 tokens of the previous request's KV (`selected slot by LCP similarity`, `n_past was set to 569` in the logs) whenever prompts shared a template prefix — and state texts share a long template prefix. Token values then differ by which ubatch alignment computed them. Fix: **one-line source patch** (`server-context.cpp`, `handle_embeddings_impl`: `task.params.cache_prompt = false;`), disclosed here and recorded in the manifest.
2. **CUDA graph capture vs replay.** First execution of a shape bucket (graph capture) returns different vectors than replays of that bucket (measured on novel-length texts: first≠second). With ~606 distinct state-text lengths, nearly every text is its own bucket — a single pass would store capture values while the recheck compares replays. Fix: **rebuilt with `-DGGML_CUDA_USE_GRAPHS=OFF`**; every request now runs the identical eager path (novel first==second verified TRUE).
3. **Memory-pool growth churn.** Early requests grow the CUDA allocator, changing pointer alignment and thus kernel-variant selection (burst of identical requests stable, but interleaved lengths differed until the pool high-water mark saturated). Fix: **long-varied throwaway warmup** (6 texts, ~3.5k tokens each) before any state text; part of the registered client.

Validation of the complete instrument (patched, graphs-off, warmed): A/B/A/C/A/B/A protocol bit-exact for all repeats; novel-length first==second bit-exact. Pass-1 and pass-2 artifacts are preserved as evidence (`*_pass1.npz/_pass2.npz` + manifests, both `determinism_recheck_bitexact: false`) and are unusable downstream — the fit asserts recheck==True on the final manifest only. No fit, metric, threshold, or model comparison has been produced at any point. The final pass is the third full 606-text execution; its result stands. Server line in the final manifest will read: `… -c 8192 -np 1 -ngl 99 --flash-attn on -b 1024 -ub 512 --cache-ram 0 --no-cache-prompt` with the patch and build flag recorded.
### 2026-09-25 — ENTRY M ADJUDICATED: quantization is EXONERATED — Qwen3-8B Q4_K_M through the llama-server stack reproduces the bf16 dial (fresh AUROC 0.9868 / Brier 0.0344 vs 0.9889 / 0.0374); the J-vs-K gap is the Gemma backbone's own representation. Diagnostic complete; it earns nothing, and it closed the question it was registered to answer.

**The run.** All instrument work happened on this laptop's RTX 5060 Laptop GPU (8 GB), zero interference with the entry-L sweep (which continued uninterrupted on the host card). Model: `Qwen_Qwen3-8B-Q4_K_M.gguf` (bartowski), sha256 `54fffa05…cccd`, verified against the upstream-published hash. Three full 606-text passes were executed (106 s final, incl. warmup); passes 1 and 2 failed the 8-row bit-exact determinism recheck and were superseded under the standing crash-fix exception — the complete three-source diagnosis, the source patch, the build flag, and the warmup protocol are disclosed in the two ledger entries above, all before any metric existed. Notable: passes 1 and 2 produced byte-identical vector files (`entry_m_emb_pass1.npz` sha == `entry_m_emb_pass2.npz` sha) — the nondeterminism is across request histories within a pass, not across identical full passes.

**The numbers (fresh 290, cold; bar for comparability only).**
| dial | encoder / stack | fresh AUROC | fresh Brier | ECE10 |
|---|---|---|---|---|
| J (reference) | Qwen3-8B bf16, HF | 0.9889 | 0.0374 | — |
| **M m_a** | **Qwen3-8B Q4_K_M, llama-server** | **0.9868** | **0.0344** | 0.0459 |
| M m_b (same, unweighted C=10) | Qwen3-8B Q4_K_M, llama-server | 0.9833 | 0.0737 | 0.1103 |
| K m_a (reference) | Gemma-4-12B nvfp4, llama-server | 0.8867 | 0.0653 | 0.0501 |

m_a: v3 oof AUROC 0.9900; m_b picked C=10 (oof Brier 0.05747). Both arms clear the comparability bar (AUROC ≥ 0.75, Brier < 0.0982). The K-b-style unweighted arm shows the same calibration fragility entry K showed — the balanced+Platt arm is the better transplant, consistently across backbones now.

**Conclusion (the attribution question, answered).** The same backbone at ~4.8 bpw through a quantized KV/quantized-weight serving stack lands within noise of the bf16 HF reference — AUROC Δ −0.002, and its Brier is actually *lower*. The earlier hypothesis ordering is now empirical: weight precision and the llama-server numerics stack are jointly exonerated; **the J-vs-K gap (0.99 → 0.89) is carried by Gemma's representation** — its last-token state is intrinsically less linearly readable for task outcome than Qwen3-8B's, on this task family. This also empirically confirms the user's prior (NVFP4-class precision ≈ int8-class quality): the ladder's intermediate rung (Q8_0, registered as follow-up) is **moot** — the question it would refine is already answered at the 4-bit rung. It stays a cheap option only if entry L forces a revisit.

**Honest status.** Diagnostic on a twice-adjudicated pool; no deployment right, nothing ships from entry M. Its one product is the attribution above plus a hardened, fully deterministic local embeddings instrument (patch + graphs-off build + warmup protocol), which is now the reference recipe for any future on-card extraction.

**Artifacts (eval/reports/entry_m/).** `entry_m_results.json` a8ee2fe7…f31b6 · `entry_m_emb.npz` 2d8e9b2c…719d · `entry_m_embed_manifest.json` f33573d3…f545 · `entry_m_fit.py` 7cd0c4d5…030f · `entry_m_embed.py` e85d0309…7f5a · `entry_m_smoke.py` 5d873ea7…a2ca6 · `entry_m_diag.py` 25ef5973…48dd · superseded evidence: `entry_m_emb_pass1.npz` 4a6f1322…ae77, `entry_m_emb_pass2.npz` 4a6f1322…ae77 (identical), manifests b0655b02…2cf1 / 0eec1f7b…706b. Fit executed once on the GPU host (hybrid-env). Teardown: local llama-server stopped by ps-verified PID; sweep processes untouched.
### 2026-09-25 — ENTRY L ADJUDICATED: GATE PASSED. The Gemma-native dial, frozen at launch and never refit, cleared both legs on 168 fresh sessions it had never seen — fresh AUROC 0.9011 (bar 0.75) and fresh Brier 0.0861 vs the pool's own constant 0.1512. The integration registration stands: the dial ships.

**The sweep (executed per the launch entry, 478cdf6).** 168/168 sessions completed, runner exited clean ("SWEEP DONE 168"), wall ≈ 11 h on the pinned infra (host 12B generation over Tailscale, local Chromium + page server). Seed plan was printed in full before session 1; the only human-visible interim signals were progress counts and page rewards — no dial output was computed until today. Raw task outcomes for context: 28/168 sessions (16.7%) earned the page's success reward.

**The pool (frozen completion-claim rule, validated on v4 counts at launch).** ckpt_review produced 955 raw claim probes across the 168 transcripts; 140 sessions fired GATE at least once and form the confirmation pool — the other 28 never fired GATE and are outside the dial's jurisdiction, exactly the disclosed contingency. Per session, the LAST probe by (turn, evidence_turn); evidence built by the rubric's own `build_evidence`; labels from page rewards. Positives: 26/140 (base rate 0.1857; constant-predictor Brier 0.1512, recomputed from labels as frozen).

**Instrument.** States file sha `a4d464ba…a79` recorded in the manifest; embeddings via the pinned host line (`--embedding --pooling last -c 32768 -np 4`, 12B nvfp4), client = one text per POST, strictly serial (the entry-K standing rule); 140 vectors in 20.1 s; 8-row determinism recheck bit-exact TRUE. Server then stopped by ps-verified PID before adjudication.

**One disclosed adjudication-script fix, BEFORE any metric existed.** The confirm script asserted its τ against the rounded display value (0.326134) and aborted on the readout artifact's full-precision τ (0.3261337492932296). Fix: the binding quantity is the sha-pinned artifact's own τ — the script now reads it from the artifact (asserting only that it rounds to the ledger value) and thresholds on it, exactly as the serving shape does. The assert aborted pre-metric; no vector, label, or threshold was touched. Fix committed with the artifacts (script sha aace0763…2bf).

**THE ONE adjudication (entry_l_confirm.py, single execution).** Readout sha verified (c840ba54…), recheck TRUE verified, states-sha chain verified; scoring in the exact serving shape (`p = σ(a·(w·x+b)+c)`, plain arithmetic; sklearn for metrics only). **Fresh AUROC 0.9011 ≥ 0.75 ✓; fresh Brier 0.0861 < 0.1512 ✓ → GATE PASS.** ECE10 0.0852. Notably the frozen dial's AUROC on truly-fresh v5 data (0.9011) exceeds its entry-K refit number on v4 (0.8867) — no degradation from the v3-only fit.

**Operating point at the frozen τ\* (what the GATE would actually route on).** tp 14 · fp 1 · tn 113 · fn 12. Read plainly: when the dial accepts a claim it is right 93.3% of the time; it catches 113 of 114 false completion claims (99.1%); but it also rejects 12 of the 26 genuine completions (46% of successes are sent back as unverified). The τ\* chosen by max-Youden-J on v3 oof landed at a conservative corner of the fresh pool. Entry L freezes this number; changing it is a NEW pre-registered decision (it trades rework for leak-rate) and is explicitly out of scope here.

**Decision, per the entry-L registration.** Gate passed ⇒ the integration registration (868219a) stands: the Gemma-native verifier dial ships as the GATE completion-claim verifier — head-as-data (3840 weights + Platt + τ\*), dot product + sigmoid + threshold at GATE claim moments, vocab demoted to logged signal. Next step is the enforce-mode integration already scoped in discussion (~1–2 days: evidence-extraction port, readout loader, retry-capped reject policy — policy to be pre-registered before it goes live), plus the optional τ\*-rebalance decision above.

**Teardown (executed).** Embeddings server stopped (PID 2010297, verified); generation server stopped (PID 1844187, verified); sweep chain proxy :8990 stopped by ps-verified PID; standing 4B proxy (:8999) and film-worker untouched; host VRAM back to baseline (551 MiB). Local sweep runner exited on its own; local page server stays up per standing rule.

**Artifacts (eval/reports/entry_l/).** entry_l_results.json bf85fca8…f6a · entry_l_emb.npz 36ba2ce6…574 · entry_l_embed_manifest.json 83ce7337…bc7 · entry_l_states.jsonl a4d464ba…a79 · entry_l_states_build.py 833bd387…de8 · entry_l_embed.py 07347cd9…aa8 · entry_l_confirm.py aace0763…2bf · readout entry_k_readout.json c840ba54…3e8. probes_v5.jsonl (955 rows) reproducible from the transcripts via the committed states-build script and left out of the repo by size.
