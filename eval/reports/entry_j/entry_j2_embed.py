...
Inputs : entry_j2_states.jsonl (fresh rows: session_dir, session, turn, label, ev)
        + the five frozen action-text pairs from the registration.
Outputs: entry_j2_emb.npz  (state_emb_raw [N,4096] fp16 L2-normed,
                           state_proj    [N,512] fp16 L2-normed,
                           act_emb_raw   [10,4096] fp16 L2-normed,
                           act_proj      [10,512] fp16 L2-normed,
                           labels [290], groups: session_dir strings)
          entry_j2_embed_manifest.json (hashes, snapshot, wall, determinism)
"""
import hashlib
import json
import time
...
rows = [json.loads(l) for l in open("entry_j2_states.jsonl")]
assert len(rows) == 290
states_sha = hashlib.sha256(open("entry_j2_states.jsonl", "rb").read()).hexdigest()
...
S = encode([r["ev"] for r in rows])   # [290, 4096] raw L2
torch.cuda.synchronize()
wall = time.time() - t0

det = bool(np.array_equal(S[:2], encode([rows[0]["ev"], rows[1]["ev"]])))

ZS = project(state_head, S)           # [290, 512]
...
np.savez("entry_j2_emb.npz", state_emb_raw=S.astype(np.float16),
...
json.dump(manifest, open("entry_j2_embed_manifest.json", "w"), indent=1)
