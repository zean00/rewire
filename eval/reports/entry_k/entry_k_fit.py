#!/usr/bin/env python3
"""Entry K adjudication — frozen recipe (ledger 2026-09-25). ONE execution.

Train side: v3 only (316 Gemma vectors, frozen folds seed 13, grouped by
session). K-a = logreg(C=10, balanced) + Platt on frozen-fold oof scores.
K-b = unweighted logreg, C in {0.01,0.1,1,10} selected by oof Brier on v3
(tie -> smaller C). Test side: 290 v4 vectors, cold, no fit touches them.
Gate per arm: AUROC >= 0.75 AND Brier < fresh-pool constant (recomputed
from v4 labels).
"""
import hashlib
import json
...
results["arms"]["k_a"] = {
    "recipe": "J-b transplanted: C=10 balanced + Platt on frozen-fold oof",
...
results["arms"]["k_b"] = {
    "recipe": "J-a transplanted: unweighted, C by v3 oof Brier, refit on all 316",
    "v3_oof_brier_by_C": {str(c): round(v, 5) for c, v in briers.items()},
    "picked_C": best_C,
}

# ---- gate on the fresh 290 ----------------------------------------------------
base = y4.mean()
const_brier = float(np.mean((base - y4) ** 2))
for arm, p in (("k_a", p_a), ("k_b", p_b)):
...
json.dump(results, open("entry_k_results.json", "w"), indent=1)
for arm in ("k_a", "k_b"):
    a = results["arms"][arm]
