#!/usr/bin/env python3
"""v5 MiniWoB sweep runner — entry L LIVE CONFIRMATION, PRE-REGISTERED (IMPLEMENTATION_PLAN.md).

Mechanics identical to the entry-E v3 runner (run_v3.py) except the four
frozen v5 deltas: a fresh CSPRNG seed RNG (9537397989, drawn
2026-09-25 at launch, before any session), exclusion of every v1-v4 seed,
fresh output paths (v5), and the throwaway sanity seed 7523903923. Task list,
prompt, arm, model string (poc-proxy-12b/gemma-12b-decide), caps, and
CDP collection are verbatim run_v3.

  python3 run_v5.py plan              # print the frozen seed plan and exit
  python3 run_v5.py sanity            # one throwaway session -> sanity_runs_v5/
  python3 run_v5.py sweep             # the frozen 168-session sweep, resumable
"""
import json
import os
import random
...
RUNS = "/tmp/mw_bench/main_runs_v5"
SANITY = "/tmp/mw_bench/sanity_runs_v5"
MAIN = "/tmp/mw_bench/main_v5.jsonl"
CDP_MAIN = "/tmp/mw_bench/main_v5.jsonl.cdp"
SEED_RNG = 9537397989
N_SEEDS = 7
OMP_CAP_S = 480
SETTLE_S = 10
...
def main():
    ...
    if mode == "plan":
        ...
        return
    if mode == "sanity":
        seed = 7523903923  # throwaway, CSPRNG-drawn at entry-L launch, outside the frozen plan
        os.makedirs(SANITY, exist_ok=True)
        global MAIN, CDP_MAIN
        MAIN = os.path.join(SANITY, "main_v5sanity.jsonl")
        CDP_MAIN = os.path.join(SANITY, "main_v5sanity.jsonl.cdp")
